...
class PRNNRec():
    ...
    def set_files_config(self, model_tag, layer_num, rnn_state_size, bottom_emb_item_len, flag_embedding_trainable,
                         folder_input, folder_output, exp_sig, init_emb_wgts_bootom_items_path, train_file, test_file,
                         train_len, test_len,
                         seq_len, batch_size, model_save_file, map_file_bottom_id_to_itemsId, top_sku_recall_file,
                         map_file_sku,
                         flag_train, train_epoch, flag_test_candidate_all, mode_attention, drop_out_r, loss_weights,
                         att_layer_cnt, bhDwellAtt, rnn_type, RNN_norm):
        # files
        ...
        [self.train_file, self.test_file, self.map_file_bottom_id_to_itemsId, self.top_sku_recall_file,
         self.map_file_sku] = \
            FileTool.add_folder_file_list(self.folder,
                                          [train_file, test_file, map_file_bottom_id_to_itemsId, top_sku_recall_file,
                                           map_file_sku])
        [self.model_save_file] = FileTool.add_folder_file_list(self.folder_output, [model_save_file])

        self.init_emb_wgts_bootom_items_path = FileTool.add_folder_file_list(self.folder,
                                                                             init_emb_wgts_bootom_items_path)

        # config
        self.model_tag = model_tag
        [self.layer_num, self.bottom_emb_item_len, self.flag_embedding_trainable, self.train_len, self.test_len,
         self.seq_len, self.batch_size] = [layer_num, bottom_emb_item_len, flag_embedding_trainable, train_len,
                                           test_len, seq_len, batch_size]
    ...
    def load_data_emb_bottom_dict(self):
        ...
        print "self.micro_item_list:", self.micro_item_list
        self.emb_wgts_micro_items_dict = Data.load_micro_item_vec(self.init_emb_wgts_bootom_items_path,
    ...
    def create_model(self):
        ...
        if (self.model_tag == "PRNN"):
            self.hie_rnn_model = PRNN.create_model_hierarchy(self.micro_item_list, self.emb_wgts_micro_items_dict,
                                                             ...
                                                             bottom_emb_item_len=self.bottom_emb_item_len,
                                                             flag_embedding_trainable=self.flag_embedding_trainable,
                                                             seq_len=self.seq_len, batch_size=self.batch_size,
                                                             mode_attention=self.mode_attention,
                                                             drop_out_r=self.drop_out_r,
                                                             att_layer_cnt=self.att_layer_cnt,
                                                             bhDwellAtt=self.bhDwellAtt, rnn_type=self.rnn_type,
                                                             RNN_norm=self.RNN_norm)
    ...
    def train_model(self):
        ...
        for e in range(n_epoch):
            ...
            for batch_id in range(batch_cnt):
                ...
                train_loss_batch = self.hie_rnn_model.train_on_batch(x_batch,
                                                                     [y_true_sku_emb, y_true_sku_emb, y_true_cid3_emb])
            ...
            info_loss = "EPOCH LOSS epoch:%d, loss:%s, avg_train_loss:%s" % (
            e + 1, str(train_loss), str(avg_train_loss))
            print info_loss
    ...
    def eval_test_result_example(self, lineNo, skuIdList, xcid3, att_sku, att_cid3, rnn_out_micro_last,
                                 dict_top_id_recallIdList, dict_emb, y_pred_emb, y_true_id, y_pred_cid3_emb,
                                 y_true_cid3_id, NList, flag_test_candidate_all):
        # cal similarity
        ...
        for candidate_id_cid3 in candidate_id_list_cid3:
            ...
            sim = np.dot(y_pred_cid3_emb, candidate_emb_cid3) / (
                        np.linalg.norm(y_pred_cid3_emb) * np.linalg.norm(candidate_emb_cid3))
    ...
    def test_model(self, NList=[1, 2, 5, 8, 10, 20, 50, 100]):
        ...
        [data_micro, total_data_line_cnt] = Data.load_data_trans_bottomId_to_microItemsId(self.test_file, nrows,
                                                                                          self.map_file_bottom_id_to_itemsId,
                                                                                          self.micro_item_list)
        ...
        for batch_id in range(batch_cnt):
            ...
            for q, out in enumerate(y_pred_sku):
                ...
                if (self.hasAttention()):
                    ...
                    self.model_output_get_dataCur_from_batch(dict_model_output_batch_data, q, lineIndex,
                                                             model_out_layerName_list)
                ...
                [res_sku, res_cid3] = self.eval_test_result_example(lineNo, xsku, xcid3, att_sku, att_cid3,
                                                                    rnn_out_micro_last, dict_top_id_recallIdList,
                                                                    self.emb_wgts_micro_items_dict, y_pred_sku_emb,
                                                                    y_true_sku_id, y_pred_cid3_emb, y_true_cid3_id,
                                                                    NList, self.flag_test_candidate_all)
                lineNo += 1
...
if __name__ == '__main__':
    model_tag = sys.argv[1]  # model名字
    layer_num = int(sys.argv[2])  # 模型层数??? 3
    rnn_state_size = sys.argv[3].split(',')  # RNN hidden size 100,100,100
    bottom_emb_item_len = int(sys.argv[4])  # ???
    flag_embedding_trainable = int(sys.argv[5])  # 1
    folder_input = sys.argv[6]
    folder_output = sys.argv[7]
    exp_sig = sys.argv[8]
    init_emb_wgts_bootom_items_path = sys.argv[9].split(',')  # embedding路径
    train_file = sys.argv[10]
    test_file = sys.argv[11]
    train_len = int(sys.argv[12])  # 577507
    test_len = int(sys.argv[13])  # 246967
    seq_len = int(sys.argv[14])  # 29
    batch_size = int(sys.argv[15])  # 128
    model_save_file = sys.argv[16]  # 存储文件
    map_file_bottom_id_to_itemsId = sys.argv[17] # mapping文件
    top_sku_recall_file = sys.argv[18]  # topn文件
    map_file_sku = sys.argv[19]  # sku mapping文件
    flag_train = sys.argv[20]  # 1
    train_epoch = int(sys.argv[21])  # epoch 2
    flag_test_candidate_all = sys.argv[22]  # 1
    mode_attention = int(sys.argv[23])  # attention模式 2
    drop_out_r = float(sys.argv[24])  # dropout 0
    loss_weights = sys.argv[25]  # loss权重 0,0.5,0.5
    att_layer_cnt = int(sys.argv[26])  # ??? 2
    bhDwellAtt = int(sys.argv[27])  # ???
    rnn_type = sys.argv[28]  # BLSTM
    RNN_norm = sys.argv[29]  # LSTM

    # model
    hieRNNRec = PRNNRec()
    hieRNNRec.set_files_config(model_tag, layer_num, rnn_state_size, bottom_emb_item_len, flag_embedding_trainable,
                               folder_input, folder_output, exp_sig, init_emb_wgts_bootom_items_path, train_file,
                               test_file, train_len, test_len,
                               seq_len, batch_size, model_save_file, map_file_bottom_id_to_itemsId, top_sku_recall_file,
                               map_file_sku,
                               flag_train, train_epoch, flag_test_candidate_all, mode_attention, drop_out_r,
                               loss_weights, att_layer_cnt, bhDwellAtt, rnn_type, RNN_norm)
