...
class FileTool():
    ...
    def read_file_to_list_list(cls, file, sep=' ', lineItemFilt=""):
        ...
        lineList = FileTool.read_line_to_list_str(file)  # 每个session读成一个string
        listList = StructureTool.format_listStr_to_listList(lineList, sep,
                                                            lineItemFilt)  # 每个session读成一个[str1, str2, ...]
